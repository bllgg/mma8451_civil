...
fileName = "AccData"+str(int(time.time()))+".csv"
...
# mydb = myclient["civilPro"]
# mycol = mydb["batchAcclData"]
...
def my_callback(channel):
    ...
    global hundredDataList
    hundredDataList.append(data)

    #collect and insert 100 data to local db
    ...
    global mycol
    if count == 100:
        # x = mycol.insert_many(hundredDataList)
        del hundredDataList[:]
        count = 0
    else :
        count += 1
...
class Accel():
    ...
    def init(self):
        ...
        self.writeRegister(REG_XYZ_DATA_CFG, self.readRegister(REG_XYZ_DATA_CFG) | FLAG_XYZ_DATA_BIT_FS_2G)  # Full Scale Range 2g, 4g or 8g
...
def getValue(data):
    ...
    y = data['y']
    z = data['z']
    precision =14  # Precision 14 bit data
    ...
    z -= signed_max if z > max_val else 0
    #
    x = round((float(x)) / (4096 / EARTH_GRAVITY_MS2), 3)
    y = round((float(y)) / (4096 / EARTH_GRAVITY_MS2), 3)
    z = round((float(z)) / (4096 / EARTH_GRAVITY_MS2), 3)
...
def TestFunc():
    ...
    cloudData = {}
    url = "http://morashmlab.projects.uom.lk/updateData"
    i = 0     
    ...
    while True:
        ...
        data['z'] = axes['z']
        f = open(fileName, "a")
        f.write(str(data['date'])+","+str(data['x'])+","+str(data['y'])+","+str(data['z'])+"\n")
        getValue(data)
        # f.write(str(axes)+"\n")
        f.flush()
        os.fsync(f)
    ...
    #    cloudData[str(i)] = data
   #
    #    if (i == 101) :
     #       r = requests.post(url, json=cloudData)
            #print(cloudData)
           # print(r.status_code)
      #      cloudData = {}       
       #     i = 0
