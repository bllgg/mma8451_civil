...
DataBuffer = []
nodeId = Node.nodeId
fileName = "Data/Acc_Data"+str(nodeId)+str(int(time.time()))+".log"
f = open(fileName, "a")
state = False
lastState = False
counter =0
...
def SyncCallBack(channel):
    ...
    # print DataBuffer
    counter =0
    fileName = "Data/Acc_Data_"+str(nodeId)+"_"+str(int(time.time()))+".log"
    # f = open(fileName, "a")
    state = not(state)
...
def GetData(signum, frame):
    ...
    if(counter<maxCounter):
        DataBuffer.append(MMA8451.getAxisValue())
